...
### Requirement: Upgrade-provider syncs the go.work go directive
`upgrade-provider.yml` SHALL include a `Sync go.work go directive` step (matching `pulumi-fivetran`'s), gated on the upgrade-branch prefixes, that sets `go.work`'s `go` directive to the maximum `go` directive across `provider/go.mod`, `provider/shim/go.mod`, and `sdk/go.mod` and commits/pushes the change when it differs. This keeps the multi-module workspace from falling behind after a bridge bump raises a module's `go` directive.

#### Scenario: go.work is bumped to match the modules
- **WHEN** the upgrade workflow runs on an upgrade branch and a module's `go` directive exceeds `go.work`'s
- **THEN** the step rewrites `go.work` to the max directive and pushes it to the bot's PR branch

#### Scenario: no-op when already in sync
- **WHEN** `go.work` already matches the max module directive
- **THEN** the step makes no commit
